...
`project-context`는 코딩 에이전트가 작업 맥락을 프로젝트 안에 계속 남기게 해서, 세션이 바뀐 뒤에도 다시 맥락을 잡고 일을 이어가기 쉽게 만드는 파일 기반 스킬입니다.

작업 중 필요한 최소한의 메모, 참고 문서, 작업 기록을 저장소 안의 평범한 파일로 남기게 해 프로젝트 문서와 작업 맥락이 쉽게 흩어지지 않도록 돕습니다. 별도 시스템 없이 사람도 바로 읽고 고치기 쉬운 단순한 구조를 지향하며, 완벽한 문서화를 목표로 하지는 않습니다.
...
- `project-context-migration` : 기존 문서가 흩어진 저장소를 처음 정리하며 도입할 때 쓰는 선택 스킬
