...
- `BRIEF.md` is the rewrite-only stable reopen brief: goal, scope, current understanding, and current output snapshot. Keep blockers, step-by-step progress, and validation history out.
- `STATUS.md` is the rewrite-only current handoff note: status, next step, blockers, declared scopes, and latest validation. Prefer no title line. Keep stable brief content and chronology out. When declared scopes are present, prefer literal `declared read scope` / `declared write scope` labels so later sessions can scan reuse boundaries quickly.
...
- Only logs are append-only. Add entries under `**YYYY-MM-DD**` headings in the current user language. Keep the latest `DECISIONS.md` block ADR-lite (usually 4 bullets). Keep the latest `WORKLOG.md` block to meaningful execution evidence another session may need; fold routine checks into `latest validation` unless they changed task state.
...
5. For most write-bearing tasks, create or update one dated task, rewrite `BRIEF.md`, `STATUS.md`, and optional `MEMORY-CANDIDATES.md` in place as current snapshots, append decisions/execution only to the logs, keep extra task-local docs only when those snapshots are not enough, and write reusable topic detail into `docs/reference/` directly when it becomes clear. Skip task creation only for very small, low-judgment, immediately-finished changes.
