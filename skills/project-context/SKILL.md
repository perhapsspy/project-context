...
- Keep disposable experiment or browser-test harnesses, run workspaces, raw run output, caches, and other execution-only artifacts outside the repository in an isolated task-scoped temporary workspace. Keep test-only browser profiles and session state separate from user environments, discard them after required inspection, and never promote them as evidence; retain only sanitized derived results when evidence is needed. Place durable code or derived evidence needed for shared review, reproduction, audit, or operation under its normal source, test, runbook, or artifact owner, even if it will be used only once.
