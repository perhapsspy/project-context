...
- A task root contains current-canonical documents and routers. Keep durable human-readable drafts, investigation notes, undecided plans, methods, and compact evidence summaries in `working/`. Create one-off execution code, run workspaces, raw output, caches, browser profiles, and session state in a repository-external temporary directory; when maintenance is in scope, promote reusable parts under their normal source or test owner. Put completed, rejected, replaced, or stale context in `archive/`.
