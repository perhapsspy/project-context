...
- `docs/reference/` holds reusable project-domain context by topic: conventions, constraints, integration patterns, and guides. Write canonical topic detail there, not in task logs. Write directly when topic curation is the task or the reusable context is already clear. If a task updates reference content, keep the path, rationale, and change trace in the task. Keep paths lowercase kebab-case and use `rg`, not an index.
