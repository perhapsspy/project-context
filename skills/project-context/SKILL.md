...
- Keep disposable experiment or browser-test harnesses, run workspaces, raw output, caches, and generated artifacts outside the repository in an isolated task-scoped temporary workspace. Keep test-only browser profiles and session state separate from user environments, and discard all such temporary state after required inspection and before the task closes. Place code or evidence needed for shared review, reproduction, audit, or operation under its normal source, test, runbook, or artifact owner, even if it will be used only once.
