...
| --- | --- | --- | --- | --- | --- |
| `AGENTS.md` | repo-local guidance | current | repo-local | `LEAVE` | 작업 기본 규칙은 source surface가 계속 소유하고 memory로 재진술하지 않는다. |
| `adr/0001-database.md` | ADR | current | project-wide | `REFERENCE` | 수용된 datastore 결론은 canonical reference에 정리하고 memory에는 다시 복사하지 않는다. |
