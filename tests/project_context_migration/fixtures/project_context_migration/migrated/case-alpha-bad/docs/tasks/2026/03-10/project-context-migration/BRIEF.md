...
| `docs/user-guide.md` | product doc | current | human-facing | `LEAVE` | 최종 사용자용 제품 문서라 agent working context로 옮기지 않는다. |
| `runbooks/deploy.md` | runbook | current | reusable | `REFERENCE -> docs/reference/deploy-runbook.md` | 현재 배포 절차의 current truth다. |
| `notes/2026-02-14-auth-spike.md` | spike note | current-but-unadopted | task-local | `TASK -> docs/tasks/2026/02-14/auth-spike/` | 탐색 메모이며 아직 production-ready가 아니고 다음 릴리스 이후 재검토가 필요하다. |
