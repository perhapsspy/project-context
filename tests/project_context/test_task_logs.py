...
class AppendLogTests(unittest.TestCase):
    def test_append_creates_new_log_with_heading_and_bullet(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "[OK] appended docs\\tasks\\2026\\04-05\\sample-task\\logs\\WORKLOG.md",
                stdout.getvalue(),
            )
            self.assertEqual(
    ...
    def test_append_rejects_missing_task_root(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("missing task root: docs\\tasks\\2026\\04-05\\missing-task", stdout.getvalue())
    ...
    def test_append_rejects_task_without_brief(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(exit_code, 1)
            self.assertIn(
                "missing task brief: docs\\tasks\\2026\\04-05\\sample-task\\BRIEF.md",
                stdout.getvalue(),
            )
    ...
    def test_append_decision_writes_one_four_line_block(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(exit_code, 0)
            self.assertIn(
                "[OK] appended docs\\tasks\\2026\\04-05\\sample-task\\logs\\DECISIONS.md",
                stdout.getvalue(),
            )
            self.assertEqual(
    ...
    def test_append_decision_rejects_malformed_latest_decision_block(self):
        with tempfile.TemporaryDirectory() as tmp:
            task_dir = make_task_dir(Path(tmp))
            decisions = task_dir / "logs" / "DECISIONS.md"
            decisions.write_text(
            ...
            with self.assertRaisesRegex(
                task_logs.LogToolError,
                "latest decision block must contain exactly 4 bullet lines",
            ):
                task_logs.append_decision_block(
                    decisions,
                    "2026-04-05",
...
class CheckCommandTests(unittest.TestCase):
    def test_check_accepts_valid_latest_block(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(exit_code, 0)
            self.assertIn(
                "[OK] latest block valid: docs\\tasks\\2026\\04-05\\sample-task\\logs\\DECISIONS.md",
                stdout.getvalue(),
            )
