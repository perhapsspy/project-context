...
- migration 스킬 계약상 audit map을 먼저 만들고 `WORK -> REFERENCE -> MEMORY -> LEAVE` 순서로 적용해야 했다.

## 옵션

- 옵션 A: 현재 사실처럼 보이는 문서를 바로 `docs/memory.md`에 흡수한다.
...
- 이후 에이전트는 `docs/memory.md`와 `docs/reference/**`만 읽어도 현재 시스템/배포 컨텍스트를 빠르게 찾을 수 있다.
